...
public class AuthEmailService {
        ...
        public void sendCreationMail(UserEntity user) {
                ResponseBodyDto emailBody = new EmailBuilder("/email/create-user.html")
                                .addSelector("${LOGIN}", user.username)
                                .addSelector(WEBSITE_NAME, websiteName)
                                .addSelector(WEBSITE_URL, websiteUrl)
                                .addSelector("${WEBSITE_ACTION}", websiteUrl + websiteUrlActivation
                                                + user.activationToken)
                                .build();

                emailService.sendEmailOutgoing(new EmailRequest(user.email,
                                websiteName + " - Activer votre compte", emailBody.content(),
                                user));
        }

        public void sendIdentityTokenMail(UserEntity user) {

                ResponseBodyDto emailBody = new EmailBuilder("/email/identity-token.html")
                                .addSelector(WEBSITE_NAME, websiteName)
                                .addSelector(WEBSITE_URL, websiteUrl)
                                .addSelector("${IDENTITY_TOKEN}", user.identityToken).build();

                emailService.sendEmailOutgoing(new EmailRequest(user.email,
                                websiteName + " - Vérification de votre identité",
                                emailBody.content(), user));
        }

        public void sendResetPassword(UserEntity user, String newPassword) {

                ResponseBodyDto emailBody = new EmailBuilder("/email/reset-password.html")
                                .addSelector(WEBSITE_NAME, websiteName)
                                .addSelector(WEBSITE_URL, websiteUrl)
                                .addSelector("${NEW_PASSWORD}", newPassword).build();

                emailService.sendEmailOutgoing(new EmailRequest(user.email,
                                websiteName + " - Nouveau mot de passe", emailBody.content(),
                                user));
        }
}
