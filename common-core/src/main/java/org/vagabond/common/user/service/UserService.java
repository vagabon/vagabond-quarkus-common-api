...
public class UserService extends BaseService<UserEntity> {
    ...
    public void doBeforeMerge(UserEntity entity, UserEntity entityNew) {
        if (StringUtils.isEmpty(entity.googleId) && StringUtils.isEmpty(entity.facebookId)) {
            try {
                if (!entity.password.equals(entityNew.password)
                        && !BcryptUtil.matches(entity.password, entityNew.password)) {
                    entity.password = AuthUtils.encrypePassword(entity.password);
                } else {
    ...
    public UserEntity addProfileToUser(UserEntity user, String profileName) {
        ...
        var profileCreatorFind = profiles.stream()
                .filter(profile -> profileName.equals(profile.name)).count();
    ...
    public List<UserResponse> findTop50(String username) {
        var query = userRepository.find("where UPPER(username) like ?1 order by username",
                QueryUtils.getLike(username));
