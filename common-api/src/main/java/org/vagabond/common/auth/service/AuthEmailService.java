...
public class AuthEmailService {
        ...
        public void sendCreationMail(UserEntity user) {
                ResponseBodyDto emailBody = new EmailBuilder("/email/create-user.html")
                                ...
                                .addSelector("${WEBSITE_ACTION}",
                                                websiteUrlActivation + user.activationToken)
                                .build();

                emailKafkaService.sendEmailOutgoing(new EmailKafkaRequest(user.email,
                                "Bingo Dingo - Activer votre compte", emailBody.content()));
        }
        ...
        public void sendIdentityTokenMail(UserEntity user) {
                ...
                emailKafkaService.sendEmailOutgoing(new EmailKafkaRequest(user.email,
                                "Bingo Dingo - Vérification de votre identité",
                                emailBody.content()));
        }
        ...
        public void sendResetPassword(UserEntity user, String newPassword) {
                ...
                emailKafkaService.sendEmailOutgoing(new EmailKafkaRequest(user.email,
                                "Bingo Dingo - Nouveau mot de passe", emailBody.content()));
        }
}
