...
class AuthResourceTest extends BaseDataTest {
	...
	void auth() {
		...
		var newUser = given().body(user)
				.header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON).when()
				.post("/auth/signup").then().statusCode(200).extract().body().as(UserEntity.class);

		newUser = given().get("/user/" + newUser.id).then().statusCode(200).extract().body()
				.as(UserEntity.class);

		// User Activation
		var activationRequest = new ActivationRequest(newUser.activationToken);
		given().body(activationRequest).header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON)
				.when().post("/auth/activation").then().statusCode(200);

		// User signin
		var authRequest = new AuthRequest("newUserAuth", "password");
		var authReponse = given().body(authRequest)
				.header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON).when()
				.post("/auth/signin").then().statusCode(200).extract().response();

		// User Refresh Token
		var refreshCookie = authReponse.cookie("refresh-token");
		given().cookie("refresh-token", refreshCookie)
				.header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON).when()
		...
		given().body(refreshTokenRequestWrong)
				.header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON).when()
				.post("/auth/refresh-token").then().statusCode(400);

		// User createIdentityToken
		var emailRequest = new AuthEmailRequest(user.email);
		given().body(emailRequest).header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON)
				.when().post("/auth/createIdentityToken");
		newUser = given().get("/user/" + newUser.id).then().statusCode(200).extract().body()
				.as(UserEntity.class);
		...
		given().body(activationRequest).header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON)
				.when().post("/auth/checkIdentityToken").then().statusCode(200);
		...
		activationRequest = new ActivationRequest(newUser.identityToken);
		given().body(activationRequest).header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON)
				.when().post("/auth/resetPassword").then().statusCode(200);

		// User logout
		given().cookie("refresh-token", refreshCookie)
				.header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON).when()
				.post("/auth/logout").then().statusCode(200);
	}
	...
	void checkAttemps() {
		...
		given().body(user).header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON).when()
				.post("/auth/signup").then().statusCode(200);

		given().body(user).header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON).when()
				.post("/auth/signup").then().statusCode(400)
		...
		given().body(authRequest).header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON)
				.when().post("/auth/signin").then().statusCode(400)
				.body("debugMessage", is("AUTH:ERROR.LOGIN_ERROR"));

		// User : wrong password
		authRequest = new AuthRequest("username2", "password2");
		given().body(authRequest).header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON)
				.when().post("/auth/signin").then().statusCode(400)
				.body("debugMessage", is("AUTH:ERROR.PASSWORD_ERROR"));

		given().body(authRequest).header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON)
				.when().post("/auth/signin").then().statusCode(400)
				.body("debugMessage", is("AUTH:ERROR.ATTEMPT_TOO_SOON"));
	}
	...
	void googleAndFacebookConnect() {
		...
		Mockito.when(httpComponent.httpGet(Mockito.anyString(), Mockito.any()))
				.thenReturn(googleResponse);
		given().body(googleRequest).header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON)
				.when().post("/auth/google-connect").then().statusCode(200);
		...
		Mockito.when(httpComponent.httpGet(Mockito.anyString(), Mockito.any()))
				.thenReturn(googleIdentityResponse);
		given().body(googleRequest).header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON)
				.when().post("/auth/google-identity-connect").then().statusCode(200);
		...
		var facebookResponse = new FacebookResponse("facebookId", "facebook@gmail.com",
				"facebookName", picture);

		Mockito.when(httpComponent.httpGet(Mockito.anyString(), Mockito.any()))
				.thenReturn(facebookResponse);
		given().body(facebookRequest).header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON)
				.when().post("/auth/facebook-connect").then().statusCode(200);

		var captchaOk = new CaptchaResponse(true);
		Mockito.when(
				httpComponent.httpPost(Mockito.anyString(), Mockito.anyString(), Mockito.any()))
				.thenReturn(captchaOk);
		given().body(facebookRequest).header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON)
				.when().post("/auth/captcha").then().statusCode(200);

		var captchaKo = new CaptchaResponse(false);
		Mockito.when(
				httpComponent.httpPost(Mockito.anyString(), Mockito.anyString(), Mockito.any()))
				.thenReturn(captchaKo);
		given().body(facebookRequest).header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON)
				.when().post("/auth/captcha").then().statusCode(400);
	}
}
