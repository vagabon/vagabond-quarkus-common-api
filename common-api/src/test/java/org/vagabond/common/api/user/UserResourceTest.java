...
class UserResourceTest extends BaseDataTest {
        ...
        @TestSecurity(user = "admin")
        void findBy() {
                given().when().get("/user/findBy?fields=username&values=username&first=0&max=10")
                                .then().statusCode(200);
        ...
        void createAndUpdateAndDelete() {
                ...
                var newUser = given().body(userRequest)
                                .header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON).when()
                                .post("/user").then().statusCode(200).extract().body()
                                .as(UserEntity.class);
                ...
                given().body(newUser).header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON)
                                .when().put("/user").then().statusCode(200);

                newUser.email = "newEmail@google.fr";
                given().body(newUser).header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON)
                                .when().put("/user/email").then().statusCode(200);
                ...
                given().body(passwordRequest)
                                .header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON).when()
                                .put("/user/password").then().statusCode(200);

                given().body(admin).header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON)
                                .when().post("/user/avatar").then().statusCode(200);

                given().header(HttpHeaders.CONTENT_TYPE, MediaType.MULTIPART_FORM_DATA)
                                .multiPart("file", FILE, "text/plain").when()
